# ============================================================
#  TreasureLife 后端「打包 + 部署」一键脚本 (PowerShell 7)
#  流程：Maven 打包 -> 删旧备份 -> 备份当前 jar -> 上传 -> 替换部署 -> 验证
#  说明：通过 SSH 免密别名连接，脚本内不含任何服务器 IP / 账号 / 密码
#  用法：
#     pwsh .qoder/skills/deploy-backend/scripts/deploy-backend.ps1
#     pwsh .qoder/skills/deploy-backend/scripts/deploy-backend.ps1 -SkipBuild   # 跳过打包，直接部署已有 jar
# ============================================================
...
# 定位项目根目录（scripts -> deploy-backend -> skills -> .qoder -> 项目根）
$ProjectRoot = (Resolve-Path (Join-Path $PSScriptRoot "..\..\..\..")).Path
$ServerDir   = Join-Path $ProjectRoot "server"
$LocalJar    = Join-Path $ServerDir "target\$JarName"
# ================================================
...
# ---------- 0. Maven 打包 ----------
if (-not $SkipBuild) {
    Write-Host "`n[0/4] Maven 打包 (prod) ..." -ForegroundColor Yellow
    Push-Location $ServerDir
    try {
        & ".\mvnw.cmd" clean package -Pprod -DskipTests
        if ($LASTEXITCODE -ne 0) { Fail "Maven 打包失败" }
    } finally {
        Pop-Location
    }
} else {
    Write-Host "`n[0/4] 已指定 -SkipBuild，跳过打包" -ForegroundColor DarkGray
}

# ---------- 前置检查 ----------
if (-not (Test-Path $LocalJar)) {
    Fail "本地 jar 不存在：$LocalJar`n    请去掉 -SkipBuild 重新打包"
}
...
Write-Host "`n[OK] 打包+部署流程执行完毕。若日志出现 'Started TreasureLifeApplication' 即启动成功。" -ForegroundColor Green
